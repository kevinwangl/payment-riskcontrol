...
风控建议与主决策规则直接绑定，在 DSL 规则命中时，Action 属性中直接携带 suggestions，避免二次规则解析开销：

```
主风控规则匹配
    │
    ▼
规则 Action = { decision: "APPROVE", suggestions: ["REQUIRE_3DS"] }
    │
    ▼
最终输出
```
...
| `entry_mode == 'CNP' AND risk_score > 40` | REVIEW | `REQUIRE_3DS` | 审核交易也建议 3DS |
...
| Chargeback 回传 | 30-180 天 | 最高（实际结果） |
| 规则自动标注 | 实时 | 中（辅助补充） |
...
风控引擎进程 (EKS)
    ├── 当前模型 (v1.2) ← 正在服务
    ├── 模型加载器
    │     ├── 接收通知 (Redis Pub/Sub 广播订阅，确保所有 Pod 同步刷新)
...
风控引擎状态检测
    ...
    └── 完全不可用
          → 所有交易拒绝（保护优先）
          → 立即触发 P0 告警
          → 记录降级期间因系统错误阻断的交易明细，产出运维与商户损失报表，便于后续核查及客诉处理
```
...
EventBridge Custom Event Bus: risk-event-bus
    ...
    │     ├──→ Amazon Data Firehose → 自动攒批/Parquet转换 → Redshift (直接流式导入)
...
- 云原生数据摄入：避免用 Lambda 写数据仓库，使用 Amazon Data Firehose 实现高吞吐免运维的 Redshift 写入
...
Transaction Service
  ...
  决策结果返回 (<50ms)
    ...
    │       ├──→ Amazon Data Firehose → 自动攒批导入 Redshift
...
- 决策日志异步写入 EventBridge → Data Firehose → Redshift
...
| SQS | 标准队列 × 9 + DLQ × 9 | ~$5 |
| Lambda | 事件处理函数 × 7（不含 Redshift 写入） | ~$20 |
