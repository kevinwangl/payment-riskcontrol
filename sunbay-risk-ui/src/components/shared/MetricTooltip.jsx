...
const metrics = {
  ...
  'REVIEW Rate': { formula: 'REVIEW Rate = (REVIEW Transactions / Total Transactions) × 100%', desc: 'Percentage of transactions flagged for manual review. High REVIEW rate increases operational cost. Target: < 10%.' },
  ...
  // Score & Level
  'Risk Score': { formula: 'Risk Score = Σ (Rule Weight × Condition Match) or ML Model Output × 100', desc: 'Transaction risk score from 0-100. Calculated by weighted rule scoring (Phase 1) or XGBoost model probability (Phase 2). Thresholds: 0-40 APPROVE, 41-70 REVIEW, 71+ DECLINE.' },
  ...
  // Decision & Action
  'Decision': { formula: 'Decision ∈ {APPROVE, REVIEW, DECLINE}', desc: 'Risk engine output. APPROVE: transaction passes. REVIEW: passes but queued for manual review. DECLINE: blocked. Any layer DECLINE = short-circuit reject. Any layer REVIEW = continue but flag.' },
  ...
  // Velocity & Lists
  'Velocity': { formula: 'Velocity = Count or Sum of txns per entity per time window', desc: 'Frequency/amount counters tracked in Redis. Examples: card txn count per 1h, IP txn count per 5m. Thresholds configured per counter, exceeding triggers REVIEW or DECLINE.' },
